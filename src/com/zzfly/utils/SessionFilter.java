	...
	public void doFilter(ServletRequest req, ServletResponse res,
		...
		for (String url : list) {
			if (servletPath.indexOf(url) > -1
					|| (servletPath.indexOf(".do") > -1 && servletPath
							.indexOf("userLogin.do") == -1)) {// 需要过滤
				logger.info("进入session过滤器->访问路径为[" + servletPath + "]");

				if (request.getSession().getAttribute("userSession") == null) {// session不存在需要拦截
					request.setAttribute("msg", "您还没有登录或登录已超时，请重新登录，然后再刷新本功能！");
					request.getRequestDispatcher("/error/noSession.jsp")
